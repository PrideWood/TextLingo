...
export function VocabularyPage({ items }: { items: KnowledgeItem[] }) {
  return <KnowledgeCategoryPage title="重点词汇" intro="从原文中抽出的词汇和短语。" items={items} emptyText="No vocabulary items generated yet." />;
}

export function ExpressionsPage({ items }: { items: KnowledgeItem[] }) {
  return <KnowledgeCategoryPage title="常用表达" intro="可以迁移到真实表达里的短语和句式。" items={items} emptyText="No expression items generated yet." />;
}

export function GrammarPage({ items }: { items: KnowledgeItem[] }) {
  return <KnowledgeCategoryPage title="语法点" intro="关注结构、时态、连接方式和可复用模式。" items={items} emptyText="No grammar items generated yet." />;
}

function KnowledgeCategoryPage({ items, intro, title, emptyText }: KnowledgeCategoryPageProps) {
  if (!items.length) {
    return (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {items.map((item) => (
          <article key={`${title}-${getKnowledgeTerm(item)}-${getKnowledgeSnippet(item)}`} className="rounded-md border border-black/10 bg-zinc-50 p-4 dark:border-white/10 dark:bg-zinc-950">
